...
func _ready() -> void:
	...
	_cb_backup_before_launch.button_pressed = Settings.read("backup_before_launch")
	_cb_backup_after_closing.button_pressed = Settings.read("backup_after_closing")
	_sb_max_auto_backups.value = Settings.read("max_auto_backups")
	
	# Connect signals for automatic backup controls
	_cb_backup_before_launch.connect("toggled", Callable(self, "_on_BackupBeforeLaunch_toggled"))
	_cb_backup_after_closing.connect("toggled", Callable(self, "_on_BackupAfterClosing_toggled"))
	_sb_max_auto_backups.connect("value_changed", Callable(self, "_on_MaxAutoBackups_value_changed"))

	# Connect keyboard input for delete key
	_list_backups.connect("gui_input", Callable(self, "_on_BackupsList_input"))
...
func _on_BtnDelete_pressed():
	...
	if backup_names.size() == 1:
		...
		await _backups.backup_batch_deletion_finished
...
func _on_BackupsList_item_selected(index):
	...
	_btn_delete.disabled = selected_items.is_empty()
	_btn_restore.disabled = selected_items.is_empty() or selected_items.size() > 1
	
	# Update info display
	if selected_items.size() == 1:
		_lbl_info.text = _make_backup_info_string(selected_items[0])
	elif selected_items.size() > 1:
		_lbl_info.text = _make_multi_selection_info_string(selected_items)
	else:
		_lbl_info.text = tr("lbl_backup_info_placeholder")
...
func _open_directory(path: String) -> void:
	...
	if OS.get_name() == "OSX":
		...
		var output: Array = []
		OS.execute("open", [path], output, false)
	else:
